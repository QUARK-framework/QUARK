...
class GreedyClassicalPVC(Solver):
    ...
    def run(self, mapped_problem: networkx.Graph, device_wrapper: any, config: any, **kwargs: dict) -> (dict, float):
        ...
        while len(mapped_problem.nodes) > 2:
            ...
            next_node = min([x for x in mapped_problem.edges(current_node[0], data=True) if  # pylint: disable=R1728
                             x[2]['c_start'] == current_node[1] and x[2]['t_start'] == current_node[2]],  # pylint: disable=R1728
                            key=lambda x: x[2]['weight'])  # pylint: disable=R1728
