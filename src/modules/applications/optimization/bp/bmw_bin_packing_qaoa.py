...
class configuration:
    def __init__(self):
        ...
        # path settings
        # is the path of this settings.py-File;  dirname returns the directory name the path
        self.path_import = os.path.dirname(os.path.dirname(__file__)) + '\\data\\import\\'
        counter = 0
        # create a string for a path with a timestamp for calculation output; the
        # %s in the string gets specified by the timefunction in the end
        self.path_export = os.path.dirname(os.path.dirname(
            __file__)) + '\\data\\export\\%s_Result_BenchQC_Opt_%s\\' % (time.strftime("%Y%m%d"), counter)
        ...
        # indicates the range of beta that will be covered in the grid-search --> [0, range_beta]
        self.range_beta = math.pi / 2
        # indicates the range of gamma that will be covered in the grid-search --> [0, range_gamma]
        self.range_gamma = 2 * math.pi
        return
...
class bin_packing_problem_docplex:
    def __init__(self, settings, bin_capacity, object_weights, coherent_objects, incompatible_objects):
        ...
        object_weights, incompatible_objects = self.join_coherent_objects(
            object_weights, coherent_objects, incompatible_objects)
        ...
        # add variables
        self.bin_variables = self.model.binary_var_list(
        ...
        self.object_to_bin_variables = self.model.binary_var_matrix(
        ...
        # add constraints
        self.assignment_constraints = self.model.add_constraints((self.model.sum(self.object_to_bin_variables[o, i] for i in range(
        ...
        self.capacity_constraints = self.model.add_constraints((self.model.sum(object_weights[o] *
                                                                               self.object_to_bin_variables[o, i] for o in range(num_of_objects)) <= bin_capacity *
                                                                self.bin_variables[i] for i in range(max_number_of_bins)), ["capacity_constraint_bin_%d" %
                                                                                                                            i for i in range(max_number_of_bins)])
        logging.info("added constraints so that the bin-capacity isn't violated")
        self.incompatibility_constraints = self.model.add_constraints((self.object_to_bin_variables[o1, i] + self.object_to_bin_variables[o2, i] <= 1 for (
            o1, o2) in incompatible_objects for i in range(max_number_of_bins)), ["incompatibility_constraint_%d" % i for i in range(max_number_of_bins * len(incompatible_objects))])
...
def transform_to_qubo(settings, bin_packing_problem):
    ...
    if settings.normed_penalty_coefficients == True:
        # to norm all the penalty-coefficients, we derive a penaltyfactor, so that
        # the largest coefficient in the objective will be
...
def create_qaoa_circuit(settings, qubo_bin_packing_problem, num_qubits_needed, theta):
    ...
    for i in range(n_layers):
        ...
        for (var1, var2), coefficient in qubo_bin_packing_problem.objective.quadratic.to_dict().items():
            if var1 == var2:
                # R_z-rotation, because x^2 = x for binary x --> is actually a linear term
                quantumcircuit_problem.rz(2 * coefficient * gamma, var1)
            else:
...
def run():
    ...
    # %%%
    logger.info(
        "we have a bin-packing-instance with: \n\t bin_capacity: %s; \n\t object_weights: %s; \n\t coherent_objects: %s; \n\t incompatible_objects: %s \n" %
    ...
    bin_packing_problem = bin_packing_problem_docplex(
        settings, bin_capacity, object_weights, coherent_objects, incompatible_objects)
    ...
    if settings.optimize_theta == True:
        ...
        logger.info(
            "The goal is to get a theta that minimizes the expected objective value of the counts of the QAOA-circuit-simulation \n")
    ...
    counts = backend.run(
        qaoa_circuit,
        seed_simulator=10,
        shots=settings.shots_to_get_feasible_solution).result().get_counts()
    ...
    for var_combination, count in counts.items():
        # transform to a list with numbers instead of strings
        var_combination_list = [float(i) for i in var_combination]
        # invert the list because qiskit variables are from right to left
        inv_var_combination_list = var_combination_list[::-1]
        ...
        # we want to derive feasible solutions, check on the transformed
        # bin-packing-problem before the qubo-transformation on feasibility
    ...
    if feasible_solutions.empty:
        ...
        logger.info(
            "By simulating the QAOA-circuit with the optimal theta, we found %s feasible solutions" %
            feasible_solutions.shape[0])
