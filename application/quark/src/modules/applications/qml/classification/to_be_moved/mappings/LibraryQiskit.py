...
class LibraryQiskit(Library):
    ...
    def select_backend(config: str) -> dict:
        ...
        if config == "cusvaer_simulator (only available in cuQuantum applicance)":
            ...
            from qiskit_braket_provider import (  # pylint: disable=C0415
                AWSBraketBackend,
                AWSBraketProvider,
            )
    ...
    def get_execute_circuit(
        circuit: QuantumCircuit, backend: Backend, config: str, n_shots: int
    ) -> callable:
        """
        ...
        if config in ["aer_statevector_simulator_gpu", "aer_statevector_simulator_cpu"]:
            ...
            def execute_circuit(solutions):
                all_circuits = [
                    circuit_transpiled.bind_parameters(solution)
                    for solution in solutions
                ]
                qobj = assemble(all_circuits, backend=backend)
                jobs = backend.run(qobj)
                pmfs = [
                    jobs.result().get_statevector(circuit).probabilities()
            ...
            def execute_circuit(solutions):
                ...
                samples_dictionary = [
                    jobs.result().get_counts(circuit).int_outcomes()
                    for circuit in all_circuits
                ]
