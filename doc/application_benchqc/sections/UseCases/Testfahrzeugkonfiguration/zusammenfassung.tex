...
\todo{Marvin: ich würde die Abhängigkeiten für Negationen umformulieren,
z.B.: "SA2 für Test ausgeschlossen"}
...
\todo{Marvin: das sollte schon vor der Optimierung sichergestellt sein.
Undurchführbare Tests sollten nicht Teil des Input-Datensatzes sein.}
...
\todo{Marvin: so kann nur je eine Bauphase gleichzeitig optimiert werden, richtig?
Sollte kein Problem sein, da wie oben beschrieben davon ausgegangen wird, dass jedem Test
genau eine Bauphase zugeordnet ist. Sollte man hier nur noch mal klarstellen.}
...
\todo{Marvin: verstehe hier den Zusammenhang nicht. Die Beschränkung des Testzeitraums auf ein Jahr
ist meiner Ansicht nach eine zusätzliche Randbedingung. Die Tatsache, dass Tests sequenziell ausgeführt
werden müssen, hat damit nicht direkt was zu tun, oder? Man könnte schreiben "Alle einem Testfahrzeug
zugewiesenen Tests müssen sequenziell innerhalb des Testzeitraums durchgeführt werden."}

Die letzten beiden Eigenschaften der Tests bezüglich der Ausführungszeit ignorieren wir zunächst.
\todo{Marvin: kann man diese Bedingungen später noch in der Bin Packing Formulierung implementieren
oder ist das modell-seitig nicht möglich. Falls dem so ist, müssen wir in Betracht ziehen, 
eine andere Formulierung zu wählen oder zumindest im Kopf zu haben, da das Scheduling der Tests
einen signifikanten Teil der Problemkomplexität ausmacht.}
