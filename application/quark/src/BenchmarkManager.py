...
class BenchmarkManager:
    ...
    def run_benchmark(self, benchmark_backlog: list, repetitions: int):
        ...
        try:
            for idx_backlog, backlog_item in enumerate(benchmark_backlog):
                config_summary = self.get_benchmark_run_configuration_summary(backlog_item)
                ...
                for i in range(1, repetitions + 1):
                    ...
                    try:
                        ...
                        _, postprocessing_time = self.application.postprocess(processed_input, None, store_dir=path,
                                                                              rep_count=i, configuration_summary=config_summary)
    ...
    def get_benchmark_run_configuration_summary(self, layered_dict: dict) -> dict:
        """
        Get from the module_submodule_dictionary that is really layered a summarizing 
        dictionary that only has the keys "problem class", "mapping", "solving method" and "solver"
        
        :param layered_dict: a dictionary that the whole procedure runs through via the different submodule-layers
        :type: layered_dict: dict
        :return: dictionary with the keys "problem class", "mapping", "solving method" and "solver"
        :rtype: dict
        """
        benchmark_run_configuration_summary = {}
        
        benchmark_run_configuration_summary['problem class'] = layered_dict['name']
        
        layered_dict = layered_dict['submodule'][next(iter(layered_dict['submodule']))]
        benchmark_run_configuration_summary['mapping'] = layered_dict['name']
        
        layered_dict = layered_dict['submodule'][next(iter(layered_dict['submodule']))]
        benchmark_run_configuration_summary['solving method'] = layered_dict['name']
        
        layered_dict = layered_dict['submodule'][next(iter(layered_dict['submodule']))]
        benchmark_run_configuration_summary['solver'] = layered_dict['name']
        
        return benchmark_run_configuration_summary
