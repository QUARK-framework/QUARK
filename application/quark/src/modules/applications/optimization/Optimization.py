...
class Optimization(Application, ABC):
    ...
    def process_solution(self, solution, **kwargs) -> (any, float):
        """
    ...
    def preprocess(self, input_data: any, config: dict, **kwargs) -> (any, float):
        ...
        if 'configuration_summary' in kwargs.keys():
            config_summary = kwargs['configuration_summary']
        else:
            config_summary = None
        
        output = self.generate_problem(config, configuration_summary=config_summary)
